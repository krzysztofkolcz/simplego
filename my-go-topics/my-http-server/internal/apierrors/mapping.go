...
import (
	...
	"github.com/krzysztofkolcz/my-http-server/internal/ptr"
...
func (m *APIErrorMapper) transform(ctx context.Context, err error) *myhttpserver.ErrorMessage {
	...
	return &myhttpserver.ErrorMessage{Error: detail}
}

// Checks if the err is in the priorityErrors group
// If true, return the ErrorMap
func (m *APIErrorMapper) containsAsPriority(err error) (*myhttpserver.ErrorMessage, bool) {
	for _, priorityErrors := range m.PriorityErrors {
		if countMatchingErrors(err, priorityErrors.Errors) > 0 {
			return &myhttpserver.ErrorMessage{Error: priorityErrors.ExposedError}, true
		}
	}
